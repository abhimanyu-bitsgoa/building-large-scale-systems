...
def main():
    ...
    try:
        ...
        report("00", "Single-node KV store", False, f"node unreachable on {N}: {e}")
    if w.status_code != 200:
        report("00", "Single-node KV store", False, f"write rejected ({w.status_code})")
    r = requests.get(f"{N}/data/{key}", timeout=10)
    ok = r.status_code == 200 and r.json().get("value") == "it-works"
    report("00", "Single-node KV store",
           ok,
