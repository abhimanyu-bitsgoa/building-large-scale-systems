...
- Makefile toolchain: `make verify|start|todo|up|down|incident|checkpoint|status` (all work).
...
5. **Full rehearsal**: a clean end-to-end pass (`make start`, walk 01→10 via todo/up/incident/
   checkpoint), confirm timing and that nothing is flaky. NOTE: `make validate` now covers the
